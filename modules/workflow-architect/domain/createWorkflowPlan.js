...
export function createWorkflowPlanJsonSchema(nodeOptions) {
  ...
  return { type: 'object', additionalProperties: false, required: ['version', 'operation', 'workflow_name', 'target_output', 'nodes', 'connections', 'input_values', 'assumptions'], properties: {
    ...
    connections: { type: 'array', maxItems: MAX_CONNECTIONS, items: { type: 'object', additionalProperties: false, required: ['from_id', 'to_id', 'to_input', 'media', 'order'], properties: { from_id: { type: 'string', minLength: 2, maxLength: 40 }, to_id: { type: 'string', minLength: 2, maxLength: 40 }, to_input: { type: 'string', minLength: 1, maxLength: 40 }, media: { type: 'string', enum: [...MEDIA] }, order: { type: 'integer', minimum: 0 } } } },
...
export function validateWorkflowPlan(plan, { nodeOptions } = {}) {
  ...
  for (const node of nodes.values()) for (const contract of options.get(node.type)?.inputs || []) {
    ...
    if (count < contract.min_connections) errors.push(issue(count === 0 ? 'PLAN_REQUIRED_INPUT' : 'PLAN_INPUT_MULTIPLICITY', `${node.id} (${node.type}) requires at least ${contract.min_connections} ${contract.media} connection${contract.min_connections === 1 ? '' : 's'} but has ${count}.`, path));
    if (contract.max_connections !== null && count > contract.max_connections) errors.push(issue('PLAN_INPUT_CARDINALITY', `${node.id} (${node.type}) accepts at most ${contract.max_connections} ${contract.media} connection${contract.max_connections === 1 ? '' : 's'} but has ${count}.`, path));
